...
#         if file_type == "csv":
#             self.file_name = f"../generated-reports/{self.when}.csv"
...
#     def save_docx(self):
#         self.docx_manager.save(f"../generated-reports/{self.when}.docx")

#     def add_pdf_report(self, data):
#             self.pdf_manager.add_report(data=data)

#     def save_pdf(self):
#         self.pdf_manager.output(f"../generated-reports/{self.when}.pdf")
...
class FileManager:
    ...
    def __init__(self, logger, file_format: str, analysis_mode="file"):
        ...
        self.mode = analysis_mode
        self.console = None
        self.when = datetime.now().strftime("%d-%b-%Y_%I-%M-%p")

        self.IS_FILE = self.mode == "file"
        self.IS_URL = self.mode == "url"
        self.IS_IP = self.mode == "ip"

        self.report_dir = Path(f"./generated-reports/{self.mode}")
        self.report_dir.mkdir(parents=True, exist_ok=True)

        self.pdf_path = self.report_dir / f"{self.when}.pdf"
        self.docx_path = self.report_dir / f"{self.when}.docx"
        self.db_path = self.report_dir / f"{self.when}.db"
    ...
    def create_file(self, file_type: str = "csv"):
        self.file_type = file_type
        self.file_name = str(self.report_dir / f"{self.when}.csv")
        ...
        if self.file_exists(self.file_name):
            ...
            try:
                ...
                self.logger.info(f"Initialized file: {self.file_name}")
                self.file_object = open(self.file_name, "a+")
            except FileNotFoundError:
                self.logger.error("Failed to create the results file!")
    ...
    def close_file(self):
        if self.file_object:
            try:
                self.file_object.close()
                self.logger.info(f"Closing file {self.file_name}\n")
            except Exception as e:
                self.logger.error(f"\nError closing file\n")
    ...
    def save_db(self):
        self.db_manager.save_db(self.db_path)

    def add_docx_report(self, data):
        self.docx_manager.add_report(data=data)

    def save_docx(self):
        self.docx_manager.save(str(self.docx_path))

    def add_pdf_report(self, data):
        self.pdf_manager.add_report(data=data)

    def save_pdf(self):
        self.pdf_manager.output(str(self.pdf_path))
